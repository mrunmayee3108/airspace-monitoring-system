...
# ---------- THREAT DETECTION ----------
if not df.empty:
    vel_threshold = df["velocity"].quantile(0.95)

    valid_alt = df[df["geo_altitude"] > 0]["geo_altitude"]
    alt_threshold = valid_alt.quantile(0.05) if len(valid_alt) > 0 else 0

    alerts = df[
        (df["velocity"] >= vel_threshold) |
        ((df["geo_altitude"] > 0) & (df["geo_altitude"] <= alt_threshold))
    ]

    alert_count = min(len(alerts), 5)
else:
    alert_count = 0

latency = round(np.random.uniform(10, 25), 1)
...
.stat-value {
    ...
    color: var(--accent-blue); line-height: 1; text-shadow: 0 0 15px rgba(0,212,255,0.4);
}
...
.section-line {
    ...
    border-radius: 2px; box-shadow: 0 0 8px var(--accent-blue);
}
.section-title {
...
.card-btn {
    display: inline-flex; align-items: center; gap: 8px;
    padding: 0.5rem 1.25rem; border: 1px solid rgba(0,212,255,0.4);
    border-radius: 7px; background: rgba(0,212,255,0.07);
    color: var(--accent-blue) !important; font-family: 'Rajdhani', sans-serif;
...
<div class="hero">
    ...
    <div style="position:relative; z-index:1;">
        ...
        <div class="hero-stats">
            <div class="stat"><div class="stat-value">{len(df) if not df.empty else 0}</div><div class="stat-label">Active Aircraft</div></div>
            <div class="stat"><div class="stat-value">{alert_count}</div><div class="stat-label">Active Alerts</div></div>
